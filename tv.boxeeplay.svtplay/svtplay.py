...
import content, utilities, logger
...
def getVideoUrl(data):
    ...
    try:
        ...
        BPTraceExit("Returning %s" %video[0])
        return video[0]
    except:
        BPLog("Exception: %s" %sys.exec_info()[0], Level.ERROR)
        BPTraceExit("Returning ''")
        return ''
	
def getDescription(data):
    BPTraceEnter(data)
    desc = re.compile('<meta property="og:description" content="([^"]+)"').findall(data, re.DOTALL) 
    BPTraceExit("Returning %s" %desc[0])
    return desc[0]
